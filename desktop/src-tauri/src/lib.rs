...
fn setup_system_tray(app: &mut tauri::App) -> tauri::Result<()> {
    let menu = MenuBuilder::new(app)
        .text(TRAY_SHOW_ID, "Show Claude Code Echoflow")
        .separator()
        .text(TRAY_QUIT_ID, "Quit Claude Code Echoflow")
        .build()?;

    let mut tray = TrayIconBuilder::with_id("main-tray")
        .tooltip("Claude Code Echoflow")
        .menu(&menu)
...
pub fn run() {
    ...
    let builder = builder
        .menu(|app| {
            let about_item =
                MenuItemBuilder::with_id("nav_about", "关于 Claude Code Echoflow").build(app)?;
            let settings_item = MenuItemBuilder::with_id("nav_settings", "设置...")
                .accelerator("CmdOrCtrl+,")
                .build(app)?;

            let app_submenu = SubmenuBuilder::new(app, "Claude Code Echoflow")
                .item(&about_item)
                .separator()
                .item(&settings_item)
                .separator()
